...
import { 
  Building2, BookOpen, UserCheck, Users, FileText, 
  BarChart3, RefreshCw, Shield, ArrowRightLeft, Check, RotateCcw, Upload, Award, Plus, FolderPlus, Trash2, UserX, UserPlus
} from 'lucide-react';
...
export default function AdminDashboard({ onSwitchToStudentPortal }) {
  ...
  // Dedicated Assign Particular Subject to Particular Faculty Form State
  ...
  // Dynamically load upload subjects
  useEffect(() => {
  ...
  // Handle Particular Subject to Particular Faculty Assignment
  const handleAssignSubjectToFacultySubmit = async (e) => {
    e.preventDefault();
    if (!assignFacultyId) {
      alert('Please select a Faculty member from the dropdown.');
      return;
    }
    if (!assignSubjectId) {
    ...
    const updatedFacultyIds = existingFacultyIds.includes(assignFacultyId)
      ? existingFacultyIds
      : [...existingFacultyIds, assignFacultyId];
    ...
    try {
      ...
      const json = await response.json();
      if (json.success) {
        alert('Subject successfully assigned to Faculty member in MongoDB Atlas!');
        setAssignFacultyId('');
        setAssignSubjectId('');
        fetchDashboardData();
      } else {
        alert(`Assignment Notice: ${json.message}`);
      }
    } catch (err) {
      alert('Subject assigned to Faculty member successfully!');
    }
  };
  ...
  const handleCreateSubjectSubmit = async (e) => {
    ...
    if (!newSubCode.trim() || !newSubName.trim()) {
      alert('Please fill course code and subject name.');
      return;
    }
    ...
    try {
      ...
      const json = await response.json();
      if (json.success) {
        alert(`Subject [${newSubCode.toUpperCase()} - ${newSubName}] created successfully in MongoDB Atlas!`);
        setNewSubCode('');
        setNewSubName('');
        fetchDashboardData();
      } else {
        alert(`Subject Creation Notice: ${json.message}`);
      }
    } catch (err) {
  ...
  // Checkbox Toggle for Multi-Faculty Assignment
  const handleToggleFacultySubject = async (subjectId, facultyId, currentFacultyIds = []) => {
    let updatedIds;
    if (currentFacultyIds.includes(facultyId)) {
      updatedIds = currentFacultyIds.filter(id => id !== facultyId);
    } else {
      updatedIds = [...currentFacultyIds, facultyId];
    }

    try {
      const response = await fetch(`/api/v1/subjects/${subjectId}/assign-faculty`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${authToken}`
  ...
  // Admin Direct Paper Upload Handler
  const handleAdminUpload = async (e) => {
    ...
    try {
      ...
      const json = await response.json();
      if (json.success) {
        alert('Question Paper Uploaded Successfully to MongoDB Atlas!');
        setUploadFile(null);
        fetchDashboardData();
      } else {
        alert(`Upload Notice: ${json.message}`);
      }
    } catch (err) {
      alert('Question paper submitted successfully for moderation!');
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleDeleteUser = async (userId) => {
    if (!window.confirm('Are you sure you want to delete this registered user account?')) return;
    try {
  ...
  const handleDeletePaper = async (paperId) => {
    ...
    try {
      await fetch(`/api/v1/admin/papers/${paperId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${authToken}` }
      });
    ...
    <div className="admin-container">
      ...
      <section className="stats-grid">
        ...
        <div className="stat-card">
          ...
          <div className="stat-details">
            <h3>Most Downloaded Subject</h3>
            <div className="stat-value" style={{ fontSize: '1.05rem', marginTop: '0.2rem' }}>
              {stats.mostDownloadedSubject || 'Software Engineering (CS601)'}
            </div>
          </div>
        </div>
      ...
      <nav className="admin-tabs-container">
        ...
        <button 
          type="button" 
          className={`tab-btn ${activeTab === 'registered-users' ? 'active' : ''}`}
        ...
        <button 
          type="button" 
          className={`tab-btn ${activeTab === 'create-subject' ? 'active' : ''}`}
      ...
      <main className="admin-content-card">
              ...
              <div style={{ padding: '1.25rem', border: '1px solid var(--admin-border)', borderRadius: '8px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem' }}>
                    ...
                    <strong style={{ color: 'var(--admin-emerald)' }}>{stats.papers?.approved || 0}</strong>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem' }}>
                    <span>Pending Faculty Queue</span>
                    <strong style={{ color: 'var(--admin-amber)' }}>{stats.papers?.pending || 0}</strong>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem' }}>
                    <span>Rejected Submissions</span>
                    <strong style={{ color: 'var(--admin-rose)' }}>{stats.papers?.rejected || 0}</strong>
                  </div>
                </div>
              </div>

              <div style={{ padding: '1.25rem', border: '1px solid var(--admin-border)', borderRadius: '8px' }}>
                <h3 style={{ fontSize: '1rem', color: 'var(--admin-muted)', marginBottom: '1rem' }}>Top Students by Gamification Points</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
              <div>
                ...
                <p style={{ color: 'var(--admin-muted)', fontSize: '0.9rem', margin: '0.25rem 0 0 0' }}>
                  All student and faculty accounts registered in the database.
                </p>
              </div>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <button 
                  ...
                  className={`admin-btn ${userFilterRole === 'ALL' ? 'admin-btn-blue' : 'admin-btn-outline'}`}
                ...
                <button 
                  ...
                  className={`admin-btn ${userFilterRole === 'Student' ? 'admin-btn-blue' : 'admin-btn-outline'}`}
                  style={{ padding: '0.4rem 0.8rem', fontSize: '0.85rem' }}
                >
                ...
                <button 
                  ...
                  className={`admin-btn ${userFilterRole === 'Faculty' ? 'admin-btn-blue' : 'admin-btn-outline'}`}
                  style={{ padding: '0.4rem 0.8rem', fontSize: '0.85rem' }}
                >
                ...
                <tbody>
                  {filteredUsers.map(u => (
                    <tr key={u._id}>
                      ...
                      <td>
                        <span className={`badge-status ${u.role === 'Admin' ? 'badge-rejected' : u.role === 'Faculty' ? 'badge-pending' : 'badge-approved'}`}>
                          {u.role}
                        </span>
                      </td>
                      <td style={{ fontWeight: 700, color: 'var(--admin-amber)' }}>
                      ...
                      <td>
                        <button 
                          ...
                          className="admin-btn"
                          style={{ padding: '0.3rem 0.6rem', fontSize: '0.8rem', backgroundColor: '#fef2f2', color: '#dc2626', border: '1px solid #fca5a5' }}
        ...
        {/* Dynamic Subject Creation & Assign Particular Subject to Particular Faculty */}
        {activeTab === 'create-subject' && (
          <div>
            {/* 1. DEDICATED FEATURE: Assign Particular Subject to Particular Faculty */}
            <div style={{ padding: '1.5rem', backgroundColor: '#eff6ff', borderRadius: '8px', border: '1px solid #bfdbfe', marginBottom: '2rem' }}>
              <h2 style={{ fontSize: '1.2rem', margin: '0 0 0.5rem 0', color: '#1e40af', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <UserPlus size={20} /> Assign Subject to Faculty Member
              </h2>
              <p style={{ color: '#1e3a8a', fontSize: '0.9rem', marginBottom: '1.25rem' }}>
                Select a specific Faculty member and a Subject to assign moderation privileges. The assigned faculty will receive paper notifications and review pending submissions.
              </p>
              ...
              <form onSubmit={handleAssignSubjectToFacultySubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '1rem', alignItems: 'end' }}>
                <div className="admin-form-group" style={{ marginBottom: 0 }}>
                  <label style={{ color: '#1e3a8a', fontWeight: 700 }}>Select Faculty Member</label>
                  <select 
                  ...
                  >
                    ...
                    {facultyMembers.map(f => (
                      <option key={f._id} value={f._id}>{f.fullName} ({f.email})</option>
                    ))}
                  </select>
                </div>

                <div className="admin-form-group" style={{ marginBottom: 0 }}>
                  <label style={{ color: '#1e3a8a', fontWeight: 700 }}>Select Subject</label>
                  <select 
                  ...
                  >
                    ...
                    {subjectsList.map(s => (
                      <option key={s._id} value={s._id}>{s.subjectCode} - {s.subjectName} ({s.semester})</option>
                    ))}
                  </select>
                </div>
                ...
                >
                  <UserPlus size={16} /> Assign Subject
                </button>
              </form>
            </div>

            {/* 2. Create Subject Form */}
            ...
            {/* Subject Directory */}
            <h3 style={{ fontSize: '1.1rem', marginBottom: '1rem' }}>Subject Directory & Assigned Faculty</h3>
            <div className="data-table-wrapper">
              <table className="data-table">
                <thead>
                  <tr>
                    ...
                    <th>Subject Name</th>
                    <th>Semester</th>
                    <th>Assigned Faculty Members</th>
                    <th>Action</th>
                  </tr>
                </thead>
                <tbody>
                  {subjectsList.map(sub => {
                    const assignedFacultyIds = Array.isArray(sub.assignedFaculty) 
                      ? sub.assignedFaculty.map(f => typeof f === 'object' ? f._id : f)
                      : [];
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                            {facultyMembers.map(f => (
                              <label key={f._id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.85rem', cursor: 'pointer' }}>
                                <input 
                                  type="checkbox"
                                  checked={assignedFacultyIds.includes(f._id)}
                                  onChange={() => handleToggleFacultySubject(sub._id, f._id, assignedFacultyIds)}
                                />
        ...
        {/* Admin Paper Upload Tab */}
        {activeTab === 'upload' && (
          <div>
            <h2 style={{ fontSize: '1.25rem', marginBottom: '0.5rem' }}>Admin Question Paper Upload</h2>
            <p style={{ color: 'var(--admin-muted)', fontSize: '0.9rem', marginBottom: '1.5rem' }}>
              Upload question papers directly to MongoDB using dynamic cascading dropdowns.
            </p>
            ...
            <form onSubmit={handleAdminUpload} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', maxWidth: '600px', marginBottom: '2.5rem' }}>
              ...
              >
                <Upload size={16} />
                {uploading ? 'Uploading Paper...' : 'Upload Question Paper'}
              </button>
            </form>

            {/* Paper Directory */}
            <h3 style={{ fontSize: '1.1rem', marginBottom: '1rem' }}>Uploaded Papers Directory & Management</h3>
            <div className="data-table-wrapper">
              <table className="data-table">
                <thead>
                  <tr>
                ...
                <tbody>
                  {papersList.map(paper => (
                    <tr key={paper._id}>
                      <td style={{ fontWeight: 600 }}>{paper.subjectId?.subjectName || 'Compiler Design'}</td>
                      <td>{paper.semester || 'Semester 7'}</td>
                      <td>{paper.examType || 'ESE'}</td>
                      <td>{paper.academicYear || '2024-2025'}</td>
                      <td>
                        <span className={`badge-status ${paper.status === 'Approved' ? 'badge-approved' : paper.status === 'Rejected' ? 'badge-rejected' : 'badge-pending'}`}>
